...
export const ShuffleHero = ({ 
  ...
  buttonText?: string;
  onButtonClick?: () => void;
  squareData?: Array<{ id: number; src: string; title?: string; }>;
}) => {
  return (
...
const shuffle = (array: Array<{ id: number; src: string; title?: string; }>) => {
...
const generateSquares = (data: Array<{ id: number; src: string; title?: string; }>) => {
  return shuffle(data).map((sq) => (
    <motion.div
      key={sq.id}
      layout
      transition={{ duration: 1.5, type: "spring" }}
      className="w-full h-full rounded-md overflow-hidden bg-muted relative group cursor-pointer"
      style={{
